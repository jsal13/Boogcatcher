...
function locsAndItemsParser(txt) {
  ...
  json.forEach((it) => {
    if (importantItems.includes(it.item)) {
      const loc = roomMap[friendlyRoomToRoomMap[it.location]["SceneName"]];

      if (!(loc in window.locsAndItems)) {
        window.locsAndItems[loc] = [it.item];
      } else {
        window.locsAndItems[loc].push(it.item);
      };

      // console.log(`${it.item} ${roomMap[friendlyRoomToRoomMap[it.location]["SceneName"]]}`);
    }
  })
  console.log(window.locsAndItems);
...
const roomMap = {
  ...
  "Dream_NailCollection": "Resting_Grounds",
  ...
  "Room_ruinhouse": "Forgotten_Crossroads",
  "Room_shop": "Dirtmouth",
